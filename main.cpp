...
void functionHandler(vector<string> source, int loc, int max_len){
  ...
  int addr_offset = -4;
  tempstr = temstr.substr(tempstr.find('(')+1, tempstr.length() - f1.function_name.length() - 3);
    ...
    for (auto & var : f1.variables) {
      ...
      if(number_of_parameter <= 6){
        if(var.type == "int"){
          ...
          f1.assembly.instructions.push_back(add_mov_instruction("%" + register_for_argument_32[number_of_parameter-1], to_string(var.addr_offset)+"(%rbp)", 32));
        }else{
          // this parameter is a 64 bits variable
          f1.assembly.instructions.push_back(add_mov_instruction("%" + register_for_argument_64[number_of_parameter-1], to_string(var.addr_offset)+"(%rbp)", 64));
        }
  ...
  while(loc < max_len){
    if((source[loc].find("int")==0 || source[loc].find("void")==0) && source[loc].find("}")==source[loc].length()-1)){
      ...
      commonInstructionHandlerDispatcher(source, loc, max_len, f1, addr_offset);
    }
  }

  if(f1.is_leaf_function == false && f1.variables.size() > 0){
    int last_offset = 0 - f1.variable.back().addr_offset;
...
void common_instruction_handler_dispatcher(string *source, int &loc, int max_len, Function &f1, int &addr_offset)
{
    ...
    */
    ...
    {
        f1.assembly_instrucions.push_back("#" + source[loc]);
...
vector loadFile(string filename, int & maxlen){
  ifstream inputFile;
...
int main(){
    ...
    functionHandler(source, 0, max_len);
